...
@connect(({ selectCommercial,loading }) => ({
  selectCommercial,
  loading: loading.effects['selectCommercial/getTableInfo'],
}))
class SelectCommercial extends Component {
  ...
  getCondition = () => {
    ...
    dispatch({
      type: 'selectCommercial/getCondition',
    });
  };
  ...
  getTableInfo = () => {
    ...
    dispatch({
      type: 'selectCommercial/getTableInfo',
      payload: values,
      callback: (res) => {
        if (res) {
  ...
  customColumns = ()=>{
    const { selectCommercial } = this.props;
    const {columns} = selectCommercial;
  ...
  customListInfo = () =>{
    const { selectCommercial } = this.props;
    const {data} = selectCommercial;
    
    return columns;
  }

  render() {
    const { from,loading,selectCommercial } = this.props;
    const { showSearchAd, selectedRows,visible , selectedKey,type } = this.state;
    const {data,columns,infoFields, orderTakerInfoColumns,orderTakerInfoDetail} = selectCommercial;
    return (
      <div>
        <PageTop
        ...
        <SearchTab
          ...
          {visible&&<AddCommercialDialog
            visible={visible}
            commercialId={selectedKey}
            type={type}
...
export default Form.create()(SelectCommercial);
