...
@connect(({ selectFeeType,loading }) => ({
  selectFeeType,
  loading: loading.effects['selectFeeType/getTableInfo'],
}))
class SelectFeeType extends Component {
  ...
  getCondition = () => {
    ...
    dispatch({
      type: 'selectFeeType/getCondition',
    });
  };
  ...
  getTableInfo = () => {
    ...
    dispatch({
      type: 'selectFeeType/getTableInfo',
      payload: values,
      callback: (res) => {
        if (res) {
  ...
  customColumns = ()=>{
    const { selectFeeType } = this.props;
    const {columns} = selectFeeType;
  ...
  customListInfo = () =>{
    const { selectFeeType } = this.props;
    const {data} = selectFeeType;
    
    return columns;
  }

  render() {
    const { from,loading,selectFeeType } = this.props;
    const { showSearchAd, selectedRows,visible , selectedKey,type } = this.state;
    const {data,columns,infoFields, orderTakerInfoColumns,orderTakerInfoDetail} = selectFeeType;
    return (
      <div>
        <PageTop
        ...
        <SearchTab
          ...
          {visible&&<AddFeeTypeDialog
            visible={visible}
            feeTypeId={selectedKey}
            type={type}
...
export default Form.create()(SelectFeeType);
