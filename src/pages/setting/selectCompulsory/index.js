...
import AddCompulsoryDialog from './components/addCompulsoryDialog';

const FormItem = Form.Item;

@connect(({ selectCompulsory, loading }) => ({
  selectCompulsory,
  loading: loading.effects['selectCompulsory/getTableInfo'],
}))
class SelectCompulsory extends Component {
  ...
  getCondition = () => {
    ...
    dispatch({
      type: 'selectCompulsory/getCondition',
    });
  };
  ...
  getTableInfo = () => {
    ...
    dispatch({
      type: 'selectCompulsory/getTableInfo',
      payload: values,
      callback: (res) => {
        if (res) {
  ...
  customColumns = () => {
    const { selectCompulsory } = this.props;
    const { columns } = selectCompulsory;
  ...
  customListInfo = () => {
    const { selectCompulsory } = this.props;
    const { data } = selectCompulsory;

    return columns;
  }

  render() {
    const { from, loading, selectCompulsory } = this.props;
    const { showSearchAd, selectedRows, visible, selectedKey, type } = this.state;
    const { data, columns, infoFields, orderTakerInfoColumns, orderTakerInfoDetail } = selectCompulsory;
    return (
      <div>
        ...
        >
          ...
          >
            ...
            {visible && <AddCompulsoryDialog
              visible={visible}
              compulsoryId={selectedKey}
              type={type}
