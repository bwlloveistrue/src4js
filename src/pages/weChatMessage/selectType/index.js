...
import AddMessageTypeDialog from './components/addMessageTypeDialog';

const FormItem = Form.Item;

@connect(({ selectMessageType,loading }) => ({
  selectMessageType,
  loading: loading.effects['selectMessageType/getTableInfo'],
}))
class SelectMessageType extends Component {
  ...
  getCondition = () => {
    ...
    dispatch({
      type: 'selectMessageType/getCondition',
    });
  };
  ...
  getTableInfo = () => {
    ...
    dispatch({
      type: 'selectMessageType/getTableInfo',
      payload: values,
      callback: (res) => {
        if (res) {
  ...
  customColumns = ()=>{
    const { selectMessageType } = this.props;
    const {columns} = selectMessageType;
  ...
  customListInfo = () =>{
    const { selectMessageType } = this.props;
    const {data} = selectMessageType;
    
    return columns;
  }

  render() {
    const { from,loading,selectMessageType } = this.props;
    ...
    return (
      <div>
        ...
        <SearchTab
          ...
          {visible&&<AddMessageTypeDialog
            visible={visible}
            messageTypeId={selectedKey}
            type={type}
...
export default Form.create()(SelectMessageType);
